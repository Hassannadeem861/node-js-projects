const Tbldata_cash_up_det = require("../../models/Database_hosthistory_model/202307tbldata_cashup_det.model");
...
exports.create = (req, res) => {
  ...
  // Create a Tbldata_cash_up_det
  var tbldata_cash_up_det = {
  ...
  // Save Tbldata_cash_up_det in the database
  Tbldata_cash_up_det.create(tbldata_cash_up_det)
    .then((data) => {
    ...
    .catch((err) => {
      ...
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the Tbldata_cash_up_det.",
      });
    });
};

// Retrieve all Tbldata_cash_up_det from the database.
exports.findAll = (req, res) => {
  const decl00500 = req.query.decl00500;
  var condition = decl00500 ? { decl00500: { [Op.like]: `%${decl00500}%` } } : null;

  Tbldata_cash_up_det.findAll({ where: condition })
    .then((data) => {
      res.send(data);
    })
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving Tbldata_cash_up_det.",
      });
    });
};
...
exports.findOne = (req, res) => {
  ...
  Tbldata_cash_up_det.findByPk(id)
    .then((data) => {
      if (data) {
        res.send(data);
      } else {
        res.status(404).send({
          message: `Cannot find Tbldata_cash_up_det with id=${id}.`,
        });
      }
    })
    .catch((err) => {
      res.status(500).send({
        message: "Error retrieving Tbldata_cash_up_det with id=" + id,
      });
    });
};
...
exports.update = (req, res) => {
  ...
  Tbldata_cash_up_det.update(req.body, {
    where: { id: id },
  })
    .then((num) => {
      if (num == 1) {
        res.send({
          message: "Tbldata_cash_up_det was updated successfully.",
        });
      } else {
        res.send({
          message: `Cannot update Tbldata_cash_up_det with id=${id}. Maybe Tbldata_cash_up_det was not found or req.body is empty!`,
        });
      }
    })
    .catch((err) => {
      res.status(500).send({
        message: "Error updating Tbldata_cash_up_det with id=" + id,
      });
    });
};
...
exports.delete = (req, res) => {
  ...
  Tbldata_cash_up_det.destroy({
    where: { id: id },
  })
    .then((num) => {
      if (num == 1) {
        res.send({
          message: "Tbldata_cash_up_det was deleted successfully!",
        });
      } else {
        res.send({
          message: `Cannot delete Tbldata_cash_up_det with id=${id}. Maybe Tbldata_cash_up_det was not found!`,
        });
      }
    })
    .catch((err) => {
      res.status(500).send({
        message: "Could not delete Tbldata_cash_up_det with id=" + id,
      });
    });
};
...
exports.deleteAll = (req, res) => {
    ...
    .then((nums) => {
      res.send({ message: `${nums} Tbldata_cash_up_det were deleted successfully!` });
    })
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while removing all Tbldata_cash_up_det.",
      });
    });
};
...
exports.findAllPublished = (req, res) => {
    Tbldata_cash_up_det.findAll({ where: { published: true } })
    .then((data) => {
      res.send(data);
    })
    .catch((err) => {
