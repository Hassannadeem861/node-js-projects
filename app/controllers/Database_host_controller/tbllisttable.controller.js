const Tbllisttable = require("../../models/Database_host_model/tbllisttable.model");
...
// Retrieve all Tbllisttables from the database.
exports.findAll = (req, res) => {
  ...
  Tbllisttable.findAll({ where: condition })
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving Tbllisttables.",
      });
    });
};
...
// Delete all Tbllisttables from the database.
exports.deleteAll = (req, res) => {
  Tbllisttable.destroy({
    ...
    .then((nums) => {
      res.send({ message: `${nums} Tbllisttables were deleted successfully!` });
    })
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while removing all Tbllisttables.",
      });
    });
};

// Find all published Tbllisttables
