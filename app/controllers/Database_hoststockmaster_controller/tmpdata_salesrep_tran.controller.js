// tmpdata_salesrep_tran.controller.js
const Tmpdata_salesrep_tran = require("../../models/Database_hoststockmaster_model/tmpdata_salesrep_tran.model");
...
exports.create = (req, res) => {
    ...
    Tmpdata_salesrep_tran.create(newTmpDataSalesRepTran)
        .then(data => {
            res.send(data);
        })
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Some error occurred while creating the Tmpdata_salesrep_tran."
            });
        });
};

// Retrieve all Tmpdata_salesrep_tran from the database.
exports.findAll = (req, res) => {
    Tmpdata_salesrep_tran.findAll()
        ...
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Some error occurred while retrieving Tmpdata_salesrep_tran."
            });
        });
};
...
// Delete all Tmpdata_salesrep_tran from the database.
exports.deleteAll = (req, res) => {
    Tmpdata_salesrep_tran.destroy({
        ...
        .then(nums => {
            res.send({ message: `${nums} Tmpdata_salesrep_tran were deleted successfully!` });
        })
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Some error occurred while removing all Tmpdata_salesrep_tran."
            });
        });
};
